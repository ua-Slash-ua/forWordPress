        ...
        try:
            ...
            if part_path not in data_func:
                if ctype in data_func:
                    index_start = data_func.find(f'{ctype}') + len(ctype)
                    index_start = data_func[index_start:].find('//') + index_start - 2
                    # index_start = data_func[:index_start].rfind(';') + 2
                    data_func = data_func[:index_start] + f'\n{inc_line}' + data_func[index_start:]
