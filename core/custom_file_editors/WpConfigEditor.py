...
class WpConfigEditor:
    ...
    def edit_custom_values(self, data):
        try:
            ...
            end_phrase = f'/* That'
            data_wp_config = self.file_manager.read_file(self.path_to_wp_config)
            index_start, index_end = self.__get_wp_config_index(data_wp_config, start_phrase, end_phrase)
            new_data = data_wp_config[:index_end-1] + '\n' +new_data + '\n' * 2 + data_wp_config[index_end:]
            self.file_manager.write_file(self.path_to_wp_config, new_data)
        except Exception as e:
            self.error_log(e)
...
if __name__ == '__main__':
    ce = WpConfigEditor(r'D:\Programms\OSPanel\home\Example\wp-config.php')
    ce.edit_wp_debug()
